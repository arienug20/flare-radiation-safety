...
export function calculateMW(entries: CompositionEntry[]): number {
  const total = entries.reduce((s, e) => s + e.molPercent, 0) || 1;
...
export function calculateThermalRadiation(params: {
  ...
  // Heat release rate
  const volumetricFlowNormal = flowRate / density; // Nm³/hr
  const Q_release = volumetricFlowNormal * lhv * 1000 * combustionEfficiency / 3600; // kW
  ...
  const actualGasVelocity = actualVolFlow / (Math.PI / 4 * calcTipDiameter * calcTipDiameter);
  ...
  // Flame tilt (API 521)
  const tiltAngle = Math.atan(windSpeed / actualGasVelocity * 0.4) * 180 / Math.PI;
...
export function calculateDispersion(params: {
  ...
  const sc = sigmaCoeffs[stabilityClass] || sigmaCoeffs['D'];
